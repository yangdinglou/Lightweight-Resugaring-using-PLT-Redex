...
\exists i.\, \redc{(\m{CoreHead}~e_1~\ldots~e_i~\ldots~e_n)}
                  {(\m{CoreHead}~e_1~\ldots~e_i'~\ldots~e_n)}\\
e_i~\in~\m{SurfExp} \andalso \redm{e_i}{e_i''}}
{\redm{(\m{CoreHead}~e_1~\ldots~e_i~\ldots~e_n)}{(\m{CoreHead}~e_1~\ldots~e_i''~\ldots~e_n)}}
\infrule[CoreRed2]
{\mbox{$\redc{(\m{CoreHead}~e_1~\ldots~e_n)}{e'}$ and is a reduction step}}
...
% for our resuarging, then prove or discuss them.
(1)
