...
\item {\em Complicated to handle hygienic sugar}. For reverse desugaring, we need to match part of the core expression on the RHS (which could be much bigger than LHS) of the desugaring rule and to get the surface term by substitution. But when a syntactic sugar introduces variable bindings, this match-and-substitute turns out to be very complex if we consider local bindings (hygienic sugars) \cite{hygienic}.

\item {\em Inefficient in reverse desugaring.} It needs to keep checking whether reverse desugaring is applicable during the evaluation of desugared core expressions, which is costive. Moreover, the match-and-substitute for reverse desugaring is expensive particularly when the core term is big.
