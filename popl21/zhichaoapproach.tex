...
If an IFA is standard, it means there are no more composite syntactic structures in it. In the above example, for the syntactic structure of \m{if}, we substituted the IFA of the \m{if} into \m{nand} and converted it into a normal IFA. Below we will prove that it is always feasible to convert IFA to normal IFA, and give the algorithm.

\begin{mythm}[Standardizability of IFA]
\label{mythm:stdifa}
Considering an IFA of a syntactic structure, if the normal IFAs of all syntactic structures of terms contained in the IFA are known, then the IFA can be transformed into a normal IFA.
\end{mythm}



Before talking about normal IFA, we prove some properties of IFA.
...
In this way, we got an IFA of \m{CoreHead}. According to the lemma \ref{mythm:stdifa}, we can also get a normal IFA of \m{CoreHead}. 
