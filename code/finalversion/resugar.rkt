...
(define (approx-exp? exp1 exp2)
  (if (and (pair? exp1) (pair? exp2))
      (if (and (equal? (length exp1) (length exp2)) (equal? (foldl + 0
          ...
          (if (equal? (car exp1) (car exp2))
              ...
              (let ((idx (index-of (map (lambda (lst1 lst2)
                                          (if (equal? lst1 lst2) 0 1))
                                        exp1 exp2)
                                   1)))
                (approx-exp? (list-ref exp1 idx) (list-ref exp2 idx))))
          #f)
      #f))
...
#;(lightweight-resugaring (term ((store)
                               ((S comb) ((K comb) ((S comb) (I comb))) (K comb) xx yy)
                               )))

#;(lightweight-resugaring (term ((store)
                               ((SS comb) ((KK comb) ((SS comb) (II comb))) (KK comb) xx yy)
                               )))
(lightweight-resugaring (term ((store)
