...
\gcomment{Fig.4 (a) (let (x CoreExp) CoreExp); Fig.4 (b) it should be | (let (x C) CoreExp) Maybe? }
...
\gcomment{is this exp same as the Exp in Fig. 5? Maybe it is confusing here.}
...
\ycomment{For the context rules, their function is to restrict the computational order of an expression based on the \m{Head}, so they can simply mixed as described in Fig. \ref{fig:mixcontext}. And if we want to limit the unique reduction of an expression, we should modify the desugaring rules to be correct for reduction.} For example, if we have the context rules as in Fig. \ref{fig:mixcontext}, then for the expression 
\[
...
\emph{Abstraction and Coverage}: Our reduction in the mixed language has some similarities to theirs. But since our framework has no execution for the fully desugared program and no reverse desugaring, there are some differences in details. For the coverage, our approach can guarantee that every step in the core languages' sequences can be reflected in the sequence before filtering, which is similar to the original one. But the abstraction and coverage are both about which kind of intermediate expression should be outputted \gcomment{or output}. Especially, the abstraction property does not need to be strict if willing to have better coverage for some cases (as mentioned in Section 3.4 in \cite{resugaring}).
