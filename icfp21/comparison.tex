...
\begin{center}
	...
	$\downarrow$\\
	\Code{(begin (set x (+ x (+ 1 1))) x) [x:1]} {\footnotesize resugar to \Code{(inc x (+ 1 1))}}\\
	$\downarrow$\\
	\Code{(begin (set x (+ 1 (+ 1 1))) x) [x:1]} \\
	$\downarrow$\\
	\Code{(begin (set x (+ 1 2)) x) [x:1]} {\footnotesize resugar to \Code{(inc x 2)}?}\\
	$\ldots$
\end{center}
