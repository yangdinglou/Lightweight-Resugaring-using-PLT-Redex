...
\begin{subfigure}{.4\linewidth}
		\begin{flushleft}
			{\scriptsize
			% \begin{Codes}
			    \qquad\quad\;\;\;\Code{ (Odd 2)}\\
			\qquad\OneStep{ \Code{ (Even (- 2 1))}}\\
			\qquad\OneStep{ \Code{ (Even 1)}}\\
			\qquad\OneStep{ \Code{ (Odd (- 1 1))}}\\
			\qquad\OneStep{ \Code{ (Odd 0)}}\\
			\qquad\OneStep{ \Code{ \#f}}\\[2.5em]
			% \end{Codes}
			}
		\end{flushleft}
...
Recursive sugar is a kind of syntactic sugars which calls itself or each other during the expansion. For example, Fibonacci sequence is a typical recursive sugar, defined as follows.
\[
	\begin{array}{l}
		\drule{\Code{(Fib 0)}}{\Code{1}}\\
		\drule{\Code{(Fib 1)}}{\Code{2}}\\
		\drule{\Code{(Fib n)}}{\Code{(+ (Fib (- n 1)) (Fib (- n 2)))}}\\
	\end{array}
\]

As recursion is a useful feature of syntactic sugar, it is common to write recursive sugar. However, the existing syntactic sugar has a disadvantage when meeting a special case---though the semantics of sugar's RHS is correct, the sugar can be ill-formed (for traditional sugar system) because the expansion of sugar never halts. For example,
\[
\begin{array}{l}
...
\end{array}
\]
are a pair of recursive syntactic sugar.
The existing resugaring approach cannot process syntactic sugar written like this  easily, because boundary conditions are in the sugar itself.
