...
Resugaring is a powerful technique that has been proposed to resolve this problem \cite{resugaring,hygienic}. It  can automatically convert the evaluation sequences of desugared program in the core language into representative sugar's syntax in the surface language.
...
\]
on the surface, after one-step reduction, we get
\[
\Code{(let ((t \#t)) (if t t \#t))}
\]
but we do not want to further get \Code{(Or1 \#t \#t)} based on the desugaring rule of \Code{Or1}, because this \Code{let} was originally used in the surface program.

Resugaring becomes hard, if we allow sugars to be hygienic, recursive and of higher-order.
...
its evaluation rules (consist of the context rules and reduction rules) of the core language.
...
\todo{This part should be revised later.}
...
\todo{Usually, a discuss should not be considered as an important contribution. Either we omit it, or we should just talk about our extension.}
