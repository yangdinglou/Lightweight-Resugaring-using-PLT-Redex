...
In our approach, the sugar can contain some bindings, written by the core language's \m{let}. The hygiene problem only happens when binders of an expanded sugar conflict with other binders. We file them into two categories---
\begin{itemize}
...
For the case2, where the sub-expression \Code{f} is a free variable introduced by the sugar \m{Subst}, the program will firstly expand the \m{Subst} (because the only hole at $e_3$ has been a value), then hygiene problem is just about the core language---which can be easily solved by capture-avoiding substitution.
